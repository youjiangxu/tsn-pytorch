...
class UnshareBiSeqVLADModule(torch.nn.Module):
    ...
    def __init__(self, timesteps, num_centers, redu_dim, with_relu=False, activation=None):
        ...
        self.f_U_r = torch.nn.init.xavier_normal(self.f_U_r, gain=1) 
        ...
        self.b_U_r = torch.nn.init.xavier_normal(self.b_U_r, gain=1) 
        ...
        self.f_U_z = torch.nn.init.xavier_normal(self.f_U_z, gain=1) 
        ...
        self.b_U_z = torch.nn.init.xavier_normal(self.b_U_z, gain=1) 
        ...
        self.f_U_h = torch.nn.init.xavier_normal(self.f_U_h, gain=1) 
        ...
        self.b_U_h = torch.nn.init.xavier_normal(self.b_U_h, gain=1) 
    ...
    def forward(self, input):
        ...
        def compute_assignments(h_tm1, backward=False):
            ...
            if not backward:
                share_w = self.f_share_w
                share_b = self.f_share_b
                U_h = self.f_U_h
                U_z = self.f_U_z
                U_r = self.f_U_r
            else:
                share_w = self.b_share_w
                share_b = self.b_share_b
                U_h = self.b_U_h
                U_z = self.b_U_z
                U_r = self.b_U_r

            ## wx_plus_b : N*timesteps, redu_dim, H, W
            wx_plus_b = torch.nn.functional.conv2d(input_tensor, share_w, bias=share_b, stride=1, padding=0, dilation=1, groups=1)
            # print('self.batch_size', wx_plus_b.size(), self.batch_size) 
            wx_plus_b = wx_plus_b.view(self.batch_size, self.timesteps, self.num_centers, self.in_shape[2], self.in_shape[3])
            ## reshape 
